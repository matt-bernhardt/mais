...
import mais.connection as connection
import time
import os
...
class Database():
    ...
    def loadConnection(self):
        conn = {}
        conn['dbuser'] = os.getenv('MAIS_DBUSER', connection.u)
        conn['dbpwd'] = os.getenv('MAIS_DBPWD', connection.p)
        conn['dbhost'] = os.getenv('MAIS_DBHOST', connection.h)
        conn['dbschema'] = os.getenv('MAIS_DBSCHEMA', connection.d)
        return conn
